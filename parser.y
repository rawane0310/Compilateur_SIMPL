...
declaration:
    ...
    | MC_DECLARER IDENTIFICATEUR CROCH_OUV CONST_ENTIER CROCH_FERM DEUX_POINTS MC_TAB MC_DE type SEPARATEUR {
        if(add_to_symbol_table_checked($2, $9, 1, $4) == 0) {
            char min_str[20] = "0";
            char max_str[20];
            sprintf(max_str, "%d", $4 - 1);
            insererQuadruplet("BOUNDS", min_str, max_str, "");
            insererQuadruplet("ADEC", $2, "", "");
...
affectation:
    ...
    | IDENTIFICATEUR CROCH_OUV expression CROCH_FERM OP_AFFECTATION expression {
        if(!is_declared($1)) {
            YYABORT;
        }
...
boucle_pour:
    ...
    } bool_expr SEPARATEUR affectation PAR_FERM {
...
bool_expr:
    ...
    | IDENTIFICATEUR {
        ...
        char* var_type = get_type($1);
        if(strcmp(var_type, "BOOL") != 0) {
            char error[200];
            sprintf(error, "'%s' n'est pas de type BOOL", $1);
            yyerror(error);
            YYABORT;
        }
